...
if (empty($firstName) || empty($lastName) || empty($email) || empty($phone) || empty($companyName)) {
    echo json_encode([
        "status" => "error",
        "message" => "First Name Last Name and Email company Name and phone are reuqired fields."
    ]);
    exit;
}
...
$ipAddress = "122.170.23.17";//getClientIp();  //"122.167.64.251"; //
...
curl_close($ch);
    ...
    $url = 'http://localhost:8084/CRM/forms/whitepaper-save'; 
